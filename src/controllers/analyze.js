...
async function getAnalyze(req, res, next){
    try{
        condition = req.body;
        // get data
        const analyze = await knex('Food Nutrition Fact').where({FoodNameENG: condition}).select('FoodID', 'FoodNameENG', 'FoodNameTH', 'Calories', 'Fat', 'Carb', 'Protein', 'Sodium', 'Portion', )
